...
class TeamDivision {
    ...
    public static int solve(int[] arr, int n, int S1, int S2, int lenS1, int lenS2) {
        // when all the elements of the array are exhausted
        if(n<0){
        	if(lenS1-lenS2>1)
        ...
        return 
        	Math.min( 
        		solve(arr,n-1, S1, S2, lenS1, lenS2),   //exclude
        		solve(arr,n-1, S1+arr[n],S2-arr[n], lenS1+1, lenS2-1)); //include
    }
	...
	public static void main (String[] args) {
	 ...
	 //    String string = scan.nextLine();
		...
		int totalSum=459;
		int n= arr.length-1;  //n is the index of the last element
		int min = solve(arr, n , 0, totalSum, 0, n);
