...
class ThreeSumHashing { 
	...
	static boolean find3Numbers(int A[], int arr_size, int sum) {
		...
		for (int i = 0; i < arr_size - 2; i++){//first element is must not be added to HashSet

		// Find pair in subarray A[i+1..n-1] with sum equal to sum - A[i] 
//Every time a new HashSet is created,Cause we dont want it to consider or count previous values 
